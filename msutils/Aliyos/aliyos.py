...
def getTrx(sheet):
    ...
    for r in range(2, sheet.max_row + 1):
        ...
        check_dt = datetime.strptime(sheet.cell(row=r, column=1).value , '%Y-%m-%d')
        ...
        # print(accountArray[last][sheet.cell(row=r, column=14).value])
...
def getEmail(sheet):
    for accounts in accountArray:
        for id in accounts:
            ...
            for r in range(2, sheet.max_row + 1):
                # if (r == 96):
                #     print("HERE: [" + str(sheet.cell(row=r, column=83).value) + "]::[" + str(id) + "]")
                if (str(sheet.cell(row=r, column=83).value) == str(id) or str(sheet.cell(row=r, column=82).value) == str(id)):
                    # print("I am there")
                    if (sheet.cell(row=r, column=33).value != ""):
                        accounts[id][3].append(sheet.cell(row=r, column=33).value)
                        print(accounts[id][3].append(sheet.cell(row=r, column=33).value))
                    if (sheet.cell(row=r, column=75).value != ""):
                        accounts[id][3].append(sheet.cell(row=r, column=75).value)
                        print(accounts[id][3].append(sheet.cell(row=r, column=75).value))
                    if (sheet.cell(row=r, column=74).value != ""):
                        accounts[id][3].append(sheet.cell(row=r, column=74).value)
                        print(accounts[id][3].append(sheet.cell(row=r, column=74).value))
                    if (sheet.cell(row=r, column=78).value != ""):
                        accounts[id][3].append(sheet.cell(row=r, column=78).value)
                        print(accounts[id][3].append(sheet.cell(row=r, column=78).value))
                    if (sheet.cell(row=r, column=79).value != ""):
                        accounts[id][3].append(sheet.cell(row=r, column=79).value)
                        print(accounts[id][3].append(sheet.cell(row=r, column=79).value))
