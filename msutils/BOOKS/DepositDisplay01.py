...
class DepositDisplay01:
    ...
    def depositExists(self, sheet, depositName):
        for r in range(5, sheet.max_row+1):
            # sw = False
            # if str(sheet.cell(row=r, column=2).value).lower().find('etch') > -1:
            #     sw = True
            #     print("-------->>>>> " + str(depositName))
            # if str(depositName).find('etcha') > -1:
            #     print("==> (" + str(sheet.title) + ")" + str(depositName) + "::" + str(sheet.cell(row=r, column=2).value))
            if str(depositName).lower().find(str(sheet.cell(row=r, column=2).value).lower()) > -1:
                # if sw:
                #     print("HERE: (" + str(r) + ")" + str(sheet.cell(row=r, column=2).value).lower())
                return True
        # print("COMPARE: " + "[" + str(depositName.lower()) + "]")
        return False
    ...
    def createDeposit(self):
        ...
        self.workbooks.save(Profile.depositDir + '\\' + self.workingFile + ".xlsx")
        self.button01.configure(state = 'disabled')
    ...
    def mySort(self, inDict):
        ...
        for line in inDict:
            ...
            outDict[str(inDict[line]['date']) + "_$_" + line] = inDict[line]
        ...
        # for e in arrToSort:
        #     sp = e.split('_$_')
        #     outDict[str(sp[0]) + str(sp[1])] = sp[2]
        #
        # for x in outDict:
        #     print(str(x) + "::" + outDict[x])
        # #targetVal = -1
        # #outDict = dict()
        # return inDict
        #
    def showData(self):
        ...
        if newDepName[0]:
            ...
            self.button01 = tk.Button(self.frame, text=buttonText, command=partial(self.createDeposit))
            self.button01.grid(row=1, column=2, columnspan=3, padx=20, pady=10, sticky=tk.EW)
        ...
        D_count = 0
        D_begin = D_end - 20
        # print (str(D_end) + "::" + str(D_begin))
        if D_begin < 0:
            D_begin = 0

        for entry in sorted(myThing.keys()):
            # print (entry)
            if D_count < D_begin:
                # print("skipping")
                D_count += 1
            ...
            # print("using")
            line = myThing[entry]
            depNameList[line['image']] = entry.split("_$_")[1]
            ...
            lab = ''
            if '@@' in depNameList[line['image']]:
                lab = ''
            else:
                lab = depNameList[line['image']]

            label00.append(tk.Label(self.frame, text=lab, bg="teal", fg="yellow"))
            label00[len(label00)-1].grid(row=row_num, column=1, padx=4, pady=4, sticky=tk.NW)
            label00[len(label00)-1].bind("<Button-1>", partial(self.showDeposit, lab))

            label00.append(tk.Label(self.frame, text=str(line['date'])[0:10], bg="teal", fg="yellow"))
            label00[len(label00)-1].grid(row=row_num, column=3, padx=4, pady=4, sticky=tk.NW)

            x = ""
            if(line['cash']):
                x = "${:0,.2f}".format(line['cash'])
            label00.append(tk.Label(self.frame, text=x, bg="teal", fg="yellow"))
            label00[len(label00)-1].grid(row=row_num, column=5, padx=4, pady=4, sticky=tk.NW)

            x = ""
            if(line['checks']):
                x = "${:0,.2f}".format(line['checks'])
            label00.append(tk.Label(self.frame, text=x, bg="teal", fg="yellow"))
            label00[len(label00)-1].grid(row=row_num, column=7, padx=4, pady=4, sticky=tk.NW)

            x = ""
            if(line['total']):
                x = "${:0,.2f}".format(line['total'])
            label00.append(tk.Label(self.frame, text=x, bg="teal", fg="yellow"))
            label00[len(label00)-1].grid(row=row_num, column=9, padx=4, pady=4, sticky=tk.NW)

            label00.append(tk.Label(self.frame, text=line['image'], bg="teal", fg="yellow"))
            label00[len(label00)-1].grid(row=row_num, column=11, padx=4, pady=4, sticky=tk.NW)
            label00[len(label00)-1].bind("<Button-1>", partial(self.CDCommonCode.showDepositImage, line['image']))
    ...
    def getDepositFromCheckFiles(self):
        ...
        for wb in workbooks:
            ...
            for name in workbooks[wb].sheetnames:
                # print(workbooks[wb][name].cell(row=2, column=7).value)
                    ...
                    for r in range(3, workbooks[wb][name].max_row):
                        try:
                            if (str(workbooks[wb][name].cell(row=r, column=4).value).find('Total:') > -1):
                                # print(name + ":found: " + str(workbooks[wb][name].cell(row=r, column=5).value))
...
#########################################################################################
# Checks all "checks/cash" spreadsheets for this Deposit, then gather the info, and
# put the info into one new spread sheet for the entire depoit.
# This spread sheet is kept in the Depoits workbook.
#########################################################################################
                        ...
                        if (self.checkCashSwitch(switch)):
                            ...
                            for f in range(1,7):
                                try:
                                    ...
                                    recordArray['cash'][counter].append("")
                        else:
                             recordArray['checks'][counter] = []
                             for f in range(1,7):
                                try:
                                    if (str(workbooks[wb][name].cell(row=r, column=3).value).find('Total') == -1):
                                        recordArray['checks'][counter].append(str(workbooks[wb][name].cell(row=r, column=f).value))
                                except:
                                    recordArray['checks'][counter].append("")
    ...
    def accumulateDeposit(self, depSheet, checkList):
        ...
        ret.append(nextLine)
        # print("UPDATING LINE: " + str(nextLine))
    ...
    def updateDeposit(self, depSheet, vals, newDepName):
        depSheet.cell(row=newDepName[1], column=5).value = round(vals['cash'],2)
        depSheet.cell(row=newDepName[1], column=6).value = round(vals['checks'],2)
        depSheet.cell(row=newDepName[1], column=7).value = round(vals['cash'],2) + round(vals['checks'],2)
        # print(newDepName)
        depSheet.cell(row=newDepName[1], column=2).value = newDepName[0]
        depSheet.cell(row=newDepName[1], column=4).value = datetime.today().strftime('%Y-%m-%d')
